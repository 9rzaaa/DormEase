...
<script>
  ...
  (function(){
    ...
    const ANSWERS = [
      ...
      { id: 'sleepover', keys: ['sleepover','visiting','guest','stay over','stay the night','matulog','makitulog','overnight'], answer: "Overnight stays are allowed for female friends/relatives only. A ₱200 sleepover fee applies per visitor, and a letter or sleepover request form must be submitted to the Admin office prior to the stay for approval.", follow: ['visitor','rules','contact'] },      { id: 'double', keys: ['double','2 person','2 occupant','two person','for two','room for 2','dalawa','dalawang tao'], answer: "The Double Room fits 2 students. Each gets their own bed and wardrobe, plus a shared study area, private bathroom, and an aircon slot. Great for roommates!", follow: ['triple','amenities','price'] },
      ...
      { id: 'curfew', keys: ['curfew','gating time','lock time','gate time','anong oras pasok','uwi','gate close','time in','time out'], answer: "Curfew is strictly at 10:00 PM every night. If you need to go beyond curfew hours, you must submit a letter or curfew form to the Admin office informing them in advance.", follow: ['rules','visitor','security'] },      { id: 'male_visitor', keys: ['male visitor','lalaki','boyfriend','male relative','lalaking bisita','pwede ba ang lalaki','can males visit'], answer: "Male visitors are NOT allowed to go inside the units, including male relatives. Female guests are allowed provided they secure authorization from the Admin office.", follow: ['visitor','rules','contact'] },
      { id: 'visitor', keys: ['visitor','bisita','bring visitor','puwede bang magdala','pwede bang magdala','visitor policy','bisita policy','female visitor','how many visitor','visitor limit'], answer: "Only female friends and relatives are allowed as visitors — male visitors are not permitted inside units. Limit is 1-2 visitors per unit, for 1-2 hours. All visitors must register at the front desk and leave before curfew at 10:00 PM.", follow: ['male_visitor','sleepover','curfew'] },      { id: 'login_logout', keys: ['log in','log out','login','logout','sign in','sign out','mag log','logbook','guard','log in log out'], answer: "Log in/log out is required at all times. When entering and going out of the building, tenants must log in/log out with the guard for reasons of safety and security.", follow: ['curfew','security','rules'] },
      ...
      { id: 'water', keys: ['water supply','tubig','water bill','bill sa tubig','water interruption','water pressure','suplay ng tubig','sub meter','submeter'], answer: "We have a strong and reliable water supply with individual sub-meters per room, so you're billed only for your actual usage. Water bills are tracked and viewable through the DormEase app. For billing questions, contact our admin team.", follow: ['dormease','price','contact'] },      { id: 'location', keys: ['location','address','where','saan','how to get there','directions','map','navarra','sampaloc','manila','nasaan','how to go'], answer: "We are located at 1229 Navarra Street, Sampaloc, Manila — just a short walk from UST and the University Belt area. Click the map on our homepage for full directions!", follow: ['near','transport','contact'] },
      ...
      { id: 'furnished', keys: ['furnished','furniture included','semi-furnished','kasama na ba','what furniture','bed included','may bed','may lamesa','desk included','wardrobe included','inclusion','exclusion','what comes with the room'], answer: "Rooms are semi-furnished and include:\n\n✅ Bed\n✅ Wardrobe/cabinet\n✅ Study table\n✅ Bookshelf\n\nNot included (tenant must bring/provide):\n❌ Aircon unit\n❌ Other electric appliances (fridge, fan, etc.)\n\nFor Wi-Fi, you can either self-connect your own internet line, or use the dorm's connection for ₱300 per device per month.", follow: ['wifi','aircon','furniture'] },      { id: 'greeting', keys: ['hi','hello','hey','good morning','good afternoon','good evening','kumusta','kamusta','musta','magandang umaga','magandang hapon','magandang gabi'], answer: "Hi there! Welcome to Sanctissimo Rosario Ladies Dormitory. I'm here to help you with anything about our rooms, amenities, rules, and more.\n\nWhat would you like to know?", follow: ['roomtypes','price','location'] },
      ...
      { id: 'reservation', keys: ['reservation fee','deposit','advance','reserve fee','down payment','paunang bayad','reservation deposit','how much to reserve'], answer: "To reserve a room, a 1-month advance payment is required. This secures your slot once approved by the admin team.", follow: ['apply','price','contact'] },
      { id: 'contract_start', keys: ['contract start','when can i move in','start date','move in date','kailan pwede mag move in','academic year start','school year start','can i start anytime'], answer: "Contracts usually start in August, but you can move in anytime as long as a room is available — just inquire with our admin team for current openings.", follow: ['apply','price','reservation'] },
      { id: 'studyhub', keys: ['study hub','coffee shop','cafe','study spot','place to study','study place','kape','kapehan','study area nearby'], answer: "There are several coffee shops and study hubs near the dormitory, perfect for group study sessions or a change of scenery from your room. Ask our admin team for current recommendations in the area!", follow: ['near','transport','location'] },
      { id: 'ae_dorm', keys: ['a&e','a and e','ae dormitory','other branch','other building','sister dorm','another dorm','other dormitory','1239 navarra','another branch'], answer: "Yes! Check out A&E Dormitory at 1239 Navarra Street — it's another building managed by the same management team as Sanctissimo Rosario. Same trusted management, same area. Contact us at +63 917 535 9723 to ask about availability there too!", follow: ['contact','location','apply'] },
    ];
    ...
    const SUGGESTION_MAP = {
      ...
      studyhub:      { icon: '☕', text: 'Study hubs nearby' },
      ae_dorm:       { icon: '🏢', text: 'A&E Dormitory' },
      reservation:   { icon: '💰', text: 'Reservation fee' },
      contract_start:{ icon: '📅', text: 'Move-in date' }
    };
    ...
    async function handleUserMessage(text) {
      ...
      if (match) {
        ...
        showSuggestions(['roomtypes','price','ae_dorm']);
      }
    }
