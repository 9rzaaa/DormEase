...
(function () {
    ...
    function isModalOpen() {
        ...
        for (var i = 0; i < overlays.length; i++) {
            if (overlays[i].style.display === 'flex' || overlays[i].classList.contains('open')) {
                return true;
            }
        }
        if (document.getElementById('dash-lightbox') && document.getElementById('dash-lightbox').classList.contains('open')) {
            return true;
        }
        return false;
    }
    ...
    function updateStatNum(selector, value) {
    ...
    function updateStatBox(index, num, sub) {
        var boxes = document.querySelectorAll('.stat-box');
        if (!boxes[index]) return;
        var numEl = boxes[index].querySelector('.stat-num');
        var subEl = boxes[index].querySelector('.stat-sub');
        if (numEl && numEl.textContent !== String(num)) numEl.textContent = num;
        if (subEl && sub !== null && subEl.textContent !== String(sub)) subEl.textContent = sub;
    }

    function buildEmergencyBanner(latestEmergency, activeCount) {
        var banner = document.querySelector('.emergency-banner');
        if (!banner) return;
        if (latestEmergency) {
            banner.className = 'emergency-banner fade-up d4';
            banner.innerHTML =
                '<div class="emerg-ico-wrap pulse">'
                + '<img src="' + assetBase + 'panic.png" alt="">'
                + '</div>'
                + '<div class="emerg-body">'
                + '<div class="emerg-label">Emergency Report</div>'
                + '<div class="emerg-detail">' + escHtmlPoll(latestEmergency.location) + ' \u2014 ' + escHtmlPoll(latestEmergency.emergency_type) + '</div>'
                + '<div class="emerg-status">' + escHtmlPoll(latestEmergency.status) + '</div>'
                + '</div>'
                + '<button class="emerg-btn" onclick="openModal(\'emergency-modal\')">View All</button>';
        } else {
            banner.className = 'emergency-banner clear fade-up d4';
            banner.innerHTML =
                '<div class="emerg-ico-wrap">'
                + '<img src="' + assetBase + 'check.png" alt="" class="check-icon">'
                + '</div>'
                + '<div class="emerg-body">'
                + '<div class="emerg-label">Emergency Status</div>'
                + '<div class="emerg-detail">All Clear</div>'
    ...
    function buildEmergencyModal(emergencies) {
        var list = document.querySelector('.emerg-modal-list');
        if (!list) return;
        if (!emergencies || emergencies.length === 0) {
            list.innerHTML = '<div class="empty-state" style="padding:1.2rem 0;">No emergency reports found.</div>';
            return;
        }
        list.innerHTML = emergencies.map(function (e) {
            var resolved = e.is_resolved;
            return '<div style="display:flex;align-items:center;gap:.85rem;padding:.85rem 1rem;border-radius:12px;border:1.5px solid;flex-shrink:0;'
                + (resolved ? 'border-color:#5bcb8a;background:#eafbf0;' : 'border-color:var(--bright-pink);background:var(--petal);') + '">'
                + '<div style="width:40px;height:40px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;'
                + (resolved ? 'background:#d3f7e6;' : 'background:var(--baby-pink);') + '">'
                + (resolved
                    ? '<img src="' + assetBase + 'check.png" style="width:18px;height:18px;object-fit:contain;filter:brightness(0) saturate(100%) invert(27%) sepia(97%) saturate(500%) hue-rotate(100deg) brightness(90%);" alt="Resolved">'
                    : '<img src="' + assetBase + 'panic.png" style="width:18px;height:18px;object-fit:contain;filter:brightness(0) saturate(100%) invert(23%) sepia(92%) saturate(3204%) hue-rotate(329deg) brightness(95%) contrast(96%);" alt="Active">')
                + '</div>'
                + '<div style="flex:1;min-width:0;">'
                + '<div style="font-size:.88rem;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'
                + escHtmlPoll(e.location) + ' \u2014 ' + escHtmlPoll(e.emergency_type)
                + '</div>'
                + '<div style="font-size:.75rem;color:var(--ink-muted);margin-top:.18rem;">' + escHtmlPoll(e.created_at) + '</div>'
                + '</div>'
                + '<div style="font-size:.7rem;font-weight:800;padding:.22rem .7rem;border-radius:6px;border:1.5px solid;flex-shrink:0;white-space:nowrap;'
                + (resolved
                    ? 'color:#1a7a4a;border-color:#5bcb8a;background:#eafbf0;'
                    : 'color:#C4003A;border-color:var(--bright-pink);background:var(--baby-pink);')
                + '">'
                + (resolved ? 'Resolved' : escHtmlPoll(e.status.charAt(0).toUpperCase() + e.status.slice(1)))
                + '</div>'
                + '</div>';
        }).join('');
    }
    ...
    function buildActivityPanel(activities) {
        var inner = document.querySelector('#body-activity .panel-inner');
        if (!inner) return;
        var badge = document.querySelector('#panel-activity .panel-badge');
        if (badge) badge.textContent = activities.length + ' entries';
        if (!activities || activities.length === 0) {
            inner.innerHTML = '<div class="empty-state">No recent visitor activity.</div>';
            return;
        }
        inner.innerHTML = activities.map(function (log) {
            ...
            return '<div class="activity-row" onclick="openVisitorModal(this)" data-visitor=\'' + data + '\'>'
                + '<div class="activity-info">'
                + '<div class="activity-title">' + escHtmlPoll(log.visitor_name) + ' \u2014 ' + escHtmlPoll(log.status)
                + (log.tenant && log.tenant !== 'N/A' ? ' for ' + escHtmlPoll(log.tenant) : '')
                + '</div>'
                + '<div class="activity-time">' + escHtmlPoll(log.arrival) + '</div>'
                + '</div>'
                + '<div class="activity-arrow">&#8250;</div>'
                + '</div>';
        }).join('');
    ...
    function buildAnnouncementsPanel(announcements) {
        ...
        if (!inner) return;
        var badge = document.querySelector('#panel-ann .panel-badge');
        if (badge) {
            var cnt = announcements.length;
            badge.textContent = cnt + ' ' + (cnt === 1 ? 'announcement' : 'announcements');
        }
        if (!announcements || announcements.length === 0) {
            inner.innerHTML = '<div class="empty-state">No announcements yet.</div>';
            return;
        }
        var priorityClass = { low: 'priority-low', moderate: 'priority-moderate', high: 'priority-high' };
        inner.innerHTML = announcements.map(function (ann) {
            var data = JSON.stringify({
                title: ann.title,
                content: ann.content,
            ...
            var pClass = priorityClass[ann.priority] || 'priority-low';
            return '<div class="announce-item" onclick="openAnnModal(this)" data-ann=\'' + data + '\'>'
                + '<div style="display:flex;align-items:center;gap:.4rem;">'
                + '<div class="announce-title">' + escHtmlPoll(ann.title) + '</div>'
                + '<span class="priority-badge ' + pClass + '">' + escHtmlPoll(ann.priority.charAt(0).toUpperCase() + ann.priority.slice(1)) + '</span>'
                + '</div>'
                + '<div class="announce-date">' + escHtmlPoll(ann.posted_at) + '</div>'
                + '</div>';
        }).join('');
    }
    ...
    function buildNotifications(notifications) {
        var iconAssets = {
            'nav-visit': assetBase + 'nav-visit.png',
            'warn':      assetBase + 'warn.png',
            'nav-announ': assetBase + 'nav-announ.png',
            'bell':      assetBase + 'bell.png',
        };
        var card = document.querySelector('.right-col .card');
        if (!card) return;
        var h3 = card.querySelector('h3');
        var container = card.querySelector('.notif-container');
        if (!container) {
        ...
        if (!notifications || notifications.length === 0) {
            container.innerHTML = '<div class="empty-state" style="padding:1rem 0;">No new notifications.</div>';
            return;
        }
        container.innerHTML = notifications.map(function (n) {
            var iconSrc = iconAssets[n.icon] || (assetBase + 'bell.png');
            var data = JSON.stringify({
                id: n.id, type: n.type, icon: iconSrc,
                message: n.message, time: n.time, ago: n.ago, url: n.url, isRead: n.isRead,
            }).replace(/'/g, '&#039;');
            return '<div class="notif-item" onclick="handleNotifClick(event, this)" data-notif=\'' + data + '\'>'
                + '<div class="notif-ico"><img src="' + iconSrc + '" alt="" onerror="this.src=\'' + assetBase + 'bell.png\'"></div>'
                + '<div>'
                + '<div class="notif-text">' + escHtmlPoll(n.message) + '</div>'
                + '<div class="notif-time">' + escHtmlPoll(n.ago) + '</div>'
                + '</div>'
                + '</div>';
        }).join('');
    }

    function updateChartIfChanged(data) {
        var newLabels   = data.chartLabels;
        var newVisitors = data.chartVisitors;
        var activeBtn   = document.querySelector('.sw-btn.active');
        if (!activeBtn || activeBtn.textContent.trim() !== 'Visitors') return;
        if (typeof window._lastChartVisitors !== 'undefined') {
            var same = JSON.stringify(newVisitors) === JSON.stringify(window._lastChartVisitors);
            if (same) return;
        }
        window._lastChartVisitors = newVisitors;
        if (typeof switchChart === 'function') {
            var CHARTS_REF = window._CHARTS_REF;
            if (CHARTS_REF) {
                CHARTS_REF.visitors.labels   = newLabels;
                CHARTS_REF.visitors.datasets[0].data = newVisitors;
            }
            switchChart('visitors', activeBtn);
        }
    }

    function escHtmlPoll(str) {
        return (str ?? '').toString()
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    function poll() {
        ...
        fetch('{{ route("frontdesk.dashboard.data") }}', {
            headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' }
        })
        .then(function (r) { return r.json(); })
        .then(function (data) {
            updateStatBox(0, data.totalTenants, 'Currently Active');
            updateStatBox(1, data.occupiedUnits, 'Out of ' + data.totalUnits + ' total units');
            updateStatBox(2, data.visitorsToday, 'Logged in today');
            updateStatBox(3, data.activeEmergencies, 'Ongoing reports');
            buildEmergencyBanner(data.latestEmergency, data.activeEmergencies);
            buildEmergencyModal(data.allEmergencies);
            buildActivityPanel(data.recentActivities);
            buildAnnouncementsPanel(data.announcements);
            buildNotifications(data.notifications);
            updateChartIfChanged(data);
        })
        .catch(function () {})
        .finally(function () {
            pollTimer = setTimeout(poll, POLL_INTERVAL);
