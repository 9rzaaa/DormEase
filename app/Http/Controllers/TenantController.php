...
class TenantController extends Controller
{
    ...
    public function frontdeskIndex()
    {
        ...
        return view('fdtenant', [
            'tenants'        => $tenants,
            'totalTenants'   => $tenants->count(),
            'activeCount'    => $tenants->whereIn('status', ['active', 'pending'])->count(),
            'pendingCount'   => $tenants->where('status', 'pending')->count(),
            'occupiedUnits'  => $occupiedUnits,
            'vacantUnits'    => $totalUnits - $occupiedUnits,
            'totalUnits'     => $totalUnits,
            'activeOccupied' => Tenant::whereIn('status', ['active', 'pending'])->whereNotNull('room_number')->distinct('room_number')->count('room_number'),
